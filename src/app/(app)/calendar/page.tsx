          ...
          <div className="space-y-3">
            {meetupsForSelected.map(meetup => {
              ...
              return (
                <div
                  key={meetup.id}
                  onClick={() => router.push(`/tee-times/${meetup.id}`)}
                  className={`bg-dark-800 rounded-2xl border overflow-hidden transition-colors cursor-pointer active:bg-dark-700 ${
                ...
                >
                  <div className="p-5">
                    ...
                    {isOrganizer && isOpen && (
                      <button
                        onClick={(e) => { e.stopPropagation(); setInviteModalMeetup(meetup.id); setInviteSearch(''); setInviteResults([]); setInviteSent(new Set()); hapticLight() }}
